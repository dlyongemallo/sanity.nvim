...
describe("find_related_targets", function()
  it("finds other stacks within the same error", function()
    T.reset_state()
    local err = T.new_error("Race", "data race on x", "test", {
      {
        label = "Write",
    ...
    local targets = T.find_related_targets(err, "src/a.c", 10, { err })
    assert_eq(#targets, 1)
  ...
  it("finds errors sharing the same address", function()
    T.reset_state()
    local err1 = T.new_error("Race", "race on x", "test", {
      { label = "Write", frames = { { func = "w", file = "a.c", line = 1 } } },
    }, { addr = "0xDEAD" })
    local err2 = T.new_error("Race", "race on x", "test", {
      { label = "Read", frames = { { func = "r", file = "b.c", line = 2 } } },
    }, { addr = "0xDEAD" })

    local targets = T.find_related_targets(err1, "a.c", 1, { err1, err2 })
    assert_eq(#targets, 1)
  ...
  it("combines intra-error and cross-error targets", function()
    ...
    local err2 = T.new_error("Race", "other race", "test", {
  ...
  it("returns empty when no address metadata exists", function()
    T.reset_state()
    local err = T.new_error("Leak_DefinitelyLost", "leak", "test", {
  ...
  it("handles table-valued addr metadata", function()
    ...
    local err2 = T.new_error("Race", "other", "test", {
