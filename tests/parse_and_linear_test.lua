...
describe("stack capture and linear rendering", function()
  it("captures caller chain from memcheck.xml", function()
    ...
    M._reset_state()
    local count = M.parse_valgrind_xml("examples/memcheck.xml")
    assert(count and count > 0, "expected parsed errors")

    local target = nil
    for _, err in ipairs(M._errors()) do
    ...
    assert_eq(s2[1].func, "malloc")
    assert_eq(s2[2].func, "process_data")
    assert_eq(s2[3].func, "demonstrate_buffer_overflow")
    assert_eq(s2[4].func, "main")
    assert_eq(basename(s2[4].file), "demo.c")
    assert_eq(s2[4].line, 328)
  end)

  it("draws a single stack as a simple linear list", function()
    M._reset_state()
    local err = M._new_error("Race", "data race", "test", {
      {
        label = "data race",
        frames = {
    ...
    local lines = M._build_stack_content("src/w.c", 10, { err.id })
  ...
  it("draws middle frames in a linear stack with branch glyph", function()
    M._reset_state()
    local err = M._new_error("Leak_DefinitelyLost", "bytes lost", "test", {
      {
        label = "bytes lost",
        frames = {
    ...
    local lines = M._build_stack_content("demo.c", 220, { err.id })
